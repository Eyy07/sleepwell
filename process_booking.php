...
// Retrieve form data
$check_in = $_POST['check-in']; // Date selected by the user for check-in
$check_out = $_POST['check-out']; // Time selected by the user for check-out (this is treated as time, not date)
$guests = $_POST['guests'];

// Retrieve user ID from session (for registered users)
$user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : NULL;

// Prepare the SQL statement to check if the `check_in` date is already booked
$stmt_check = $conn->prepare("
    SELECT * FROM bookings
    WHERE check_in = ?
");
$stmt_check->bind_param("s", $check_in);
$stmt_check->execute();
$result = $stmt_check->get_result();

if ($result->num_rows > 0) {
    // If the check_in date is already booked, show an error message
    echo "<h1>Booking Unsuccessful</h1>";
    echo "<p>The check-in date (" . htmlspecialchars($check_in) . ") is already booked. Please choose another date.</p>";
    echo '<a href="calendar.html" class="button">Go Back</a>';
} else {
    // If no conflicts, proceed to insert the booking
    ...
    if ($stmt_insert->execute()) {
        ...
        // Store booking ID in the session for future use (like in loading.php)
        ...
        echo "<p>Check-out Time: " . htmlspecialchars($check_out) . "</p>";
